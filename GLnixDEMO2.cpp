...
bool GLnixDEMO::Init() {
    if (!GLnixAPP::Init()) return false;

    cloth.Init(200, 200, 0.025, 0.01, 3000, 2500, 1.1, 1.5,
               0.8);  // Blue velvet, I like it
...
void GLnixDEMO::UpdateScene(float dt) {
    ...
    GLnix_glBufferSubData(GL_ARRAY_BUFFER, 0,
                          cloth.VertexCount() * sizeof(GLfloat) * 11, &v[0]);
}
...
void GLnixDEMO::BuildClothGeometryBuffers() {
    ...
    GLnix_glBufferData(GL_ARRAY_BUFFER,
                       cloth.VertexCount() * sizeof(GLfloat) * 11, 0,
                       GL_STATIC_DRAW);

    GLnix_glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, BUFFERS[1]);
    GLnix_glBufferData(GL_ELEMENT_ARRAY_BUFFER, indices.size() * sizeof(UINT),
                       &indices.front(), GL_STATIC_DRAW);

    glVertexPointer(3, GL_FLOAT, sizeof(GLfloat) * 11, 0);
    glNormalPointer(GL_FLOAT, sizeof(GLfloat) * 11,
...
void GLnixDEMO::OnMouseMove(int x, int y) {
    if (but == 1) {
        ...
        float dx =
            ANTMATHConvertToRadians(0.25f * static_cast<float>(x - mousex));
        float dy =
            ANTMATHConvertToRadians(0.25f * static_cast<float>(y - mousey));
