...
void GeometryGenerator::CreateSphere(float radius, UINT sliceCount,
    ...
    for (UINT i = 0; i < stackCount - 2; ++i) {
        for (UINT j = 0; j < sliceCount; ++j) {
            ...
            meshData.Indices.push_back(baseIndex + (i + 1) * ringVertexCount +
                                       j);
    ...
    //
    // Compute indices for bottom stack.  The bottom stack was written last to
    // the vertex buffer and connects the bottom pole to the bottom ring.
    //
...
void GeometryGenerator::CreateGrid(float width, float depth, UINT m, UINT n,
                                   MeshData& meshData) {
